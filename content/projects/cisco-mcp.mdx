...
The biggest new idea for me was decoupling. I'd never decoupled anything. I didn't even know the word. My mentor said it and I genuinely went, what does that mean? It means separating the API layer from the MCP layer. In our case the FastAPI side owned the REST backend: the dashboard, the registry of tools, onboarding, auth, and generating schemas. The FastMCP side owned actually speaking MCP to the agents. They could have lived in one process, FastMCP even mounts straight into FastAPI, but keeping them as a clean boundary meant we could reason about, scale, and ship each one on its own. It also kept the MCP layer something we designed on purpose, which matters, because a hand-designed MCP server beats a blind one-to-one mirror of every REST endpoint you have.
